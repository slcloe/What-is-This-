...
public class SetLanguage : MonoBehaviour
{
	...
	void SetKorText()
	{
		...
		SetTextContent(SetText.texts[2], "학습화면으로 돌아가기");
		SetTextContent(SetText.texts[4], UserInfo.GetcntWord().ToString() + " 개");
		SetTextContent(SetText.texts[5], "오늘까지 학습한 단어");
		SetTextContent(SetText.texts[6], "현재 아이 레벨");
		SetTextContent(SetText.texts[7], "학습 결과");
		SetTextContent(SetText.texts[10], "보상 설정");
		SetTextContent(SetText.texts[11], "상품수여 주기");
		SetTextContent(SetText.texts[12], "상품 이름");
		SetTextContent(SetText.texts[13], "상품을 입력해주세요.");
		SetTextContent(SetText.texts[15], "상품수여주기를 입력해주세요.");
		SetTextContent(SetText.texts[17], "수정하기");
		SetTextContent(SetText.texts[18], "언어 설정");
		SetTextContent(SetText.texts[19], "한국어");
		SetTextContent(SetText.texts[20], "영어");
	}
